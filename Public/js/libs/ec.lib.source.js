	...
	loading : {
		...
		show : function (options) {
			...
			var thix = this,
				...
				css = {},
				maskObj = null,
				$load_ui_bg = $('<div id="'+ opt.maskId +'" class="'+ opt.maskId +'"></div>'),
			...
			//加载蒙层效果
			maskObj = gid(opt.maskId);
			if(maskObj){
				maskObj.style.display = 'block';
			} else {
				$load_ui_bg.css({"height":$(document).height()+'px', "width":"100%"}).appendTo('body').show();
				thix.setPosition();
			}
			if(!opt.modal) {
				$('#'+opt.maskId).css('opacity', 0);
			}
			$(window).on("resize", function () {
				thix.maskerResize($load_ui_bg);
				thix.setPosition();
			});
		...
		hide : function () {
			...
			$('#'+ opt.maskId +', #'+opt.loadId).hide();
		}
	},
	scrollTo : function(ele, offsetY) {
		var top = 0;
